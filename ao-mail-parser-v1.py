...
myid=emailmessage.subject.split(":")[2]
action=emailmessage.subject.split(":")[1]
log.info("Incident Id: " +str(myid))
log.info("Action: " +str(action))

# The new incident owner
newIncidentOwner = "l1-stuff@egearge.com "
...
query_builder.equals(fields.incident.id, int(myid))
...
if len(incidents) == 0:
  ...
  if action=="Approved":
    incident.properties.actionapproved="1"
    log.info("set actionapproved as 1")
    incident.addNote("Action request Approved!")
  elif action=="Denied":
    incident.properties.actionapproved="0"
    log.info("set actionapproved as 0")
    incident.addNote("Action request NOT PERMITTED!")
